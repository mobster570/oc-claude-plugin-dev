...
> **OpenCode Note:** This workflow was designed for Claude Code. `AskUserQuestion` maps to OpenCode's `question` tool (structured UI choices with labels and descriptions). `TodoWrite` maps to OpenCode's todo tracking. `Task` maps to OpenCode's `task` tool for subagent dispatch.
